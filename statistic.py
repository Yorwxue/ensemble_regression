    ...
    if os.path.exists(path):
        if os.path.isdir(path) and fold_enable == 1:
            ...
            if path.find('.ods') != -1:
                ...
                #                                         sites[siteName][month][interval]['precision'] +
                #                                         sites[siteName][month][interval]['recall']) != 0 else -1
                sites[siteName][month][interval]['precision'] = float(buffer[14][2])
                sites[siteName][month][interval]['recall'] = float(buffer[16][2])
                sites[siteName][month][interval]['f1'] = float(buffer[18][2])
...
local = ''
city = ''
site_list = ['板橋']  # 板橋 林口 萬華 新竹 苗栗 忠明 西屯 斗六 嘉義 左營 小港    # ['中山', '古亭', '士林', '松山', '萬華'] not used
month_list = ['1']  # ['1', '12']
hr = '6'
...
path = (root_path + 'result/%s/%s' % (local, city))
sites = dict()
